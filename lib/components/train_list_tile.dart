...
import 'dialogs/stoplist_dialog.dart';
...
class TrainListTile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final facility = ref.watch(forecastProvider).value?[train.facilityId]
        ?? service?.facilities[train.facilityId];
    final carType = facility?.carType ?? CarType.unfixed;
    ...
    return train.stops.isEmpty ? const SizedBox.shrink() : Padding(
      ...
      child: Material(
        ...
        child: Container(
          ...
          child: InkWell(onTap: () => showDialog(
            context: context,
            builder: (context) => StopListDialog(
              train: train, stopsNameList: stopsNameList, facility: facility
            )
          ), child: Padding(
            padding: const EdgeInsets.fromLTRB(8, 8, 7, 10),
